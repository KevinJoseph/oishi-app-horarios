...
function getMondayOfWeekISO(weekStartParam?: string): string {
  if (weekStartParam && /^\d{4}-\d{2}-\d{2}$/.test(weekStartParam)) return weekStartParam;
...
export async function getPublicEmployeeScheduleController(req: Request, res: Response): Promise<void> {
  ...
  let weekPlan: { days: unknown[]; restDayOverrides?: Record<string, number[]> } | null = null;
  let weekConfig: { timeSlots?: unknown[]; breakConfig?: Record<string, unknown> } | null = null;
  if (weekDoc) {
    const planDoc = await WeekPlanModel.findOne({ companyId, areaId, baseWeekId: weekDoc.baseWeekId }).lean();
    if (planDoc) {
      weekPlan = {
        days: planDoc.days,
        ...(planDoc.restDayOverrides ? { restDayOverrides: planDoc.restDayOverrides as Record<string, number[]> } : {})
      };
    }
  ...
  const timeSlots =
    weekConfig?.timeSlots && (weekConfig.timeSlots as unknown[]).length > 0
      ? weekConfig.timeSlots
      : areaSettings?.timeSlots && (areaSettings.timeSlots as unknown[]).length > 0
        ? areaSettings.timeSlots
        : seed.timeSlotsByArea[areaId] ?? seed.timeSlots;

  const breakConfig =
    weekConfig?.breakConfig ?? areaSettings?.breakConfig ?? seed.breakConfigByArea[areaId] ?? seed.breakConfig;

  const roleDocs = await RoleModel.find({ companyId, areaId }).lean();
  const roles = roleDocs.map((doc) => ({ ...(doc.data as Record<string, unknown>), areaId: doc.areaId }));
