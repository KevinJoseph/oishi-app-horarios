...
async function replaceRolesForCompany(companyId, roles) {
    ...
    const ops = roles.map((role) => {
        const areaId = isValidAreaCode(role.areaId) ? role.areaId : '';
        return {
            updateOne: {
                filter: { _id: role.id },
                update: {
...
export async function replacePlannerState(context, payload) {
    ...
    await Promise.all([
        ...
        AppSettingsModel.updateOne({ _id: APP_SETTINGS_ID }, { $set: { currentAreaId: payload.currentAreaId?.trim() ? payload.currentAreaId : null } }, { upsert: true }),
